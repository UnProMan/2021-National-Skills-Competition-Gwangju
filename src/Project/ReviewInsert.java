...
public class ReviewInsert extends JDialog implements Base{
	...
	JButton btn1 = get(new JButton("등록"));
	...
	public ReviewInsert(ArrayList<String> data) {
		
		this.data = data;
		
		SetDial(this, "리뷰 등록", DISPOSE_ON_CLOSE, 350, 400);
		design();
		action();
		setVisible(true);
	...
	public void design() {
		...
		for (int i = 0; i < lab.length; i++) {
			p3.add(lab[i] = get(new JLabel("☆"), setf(Color.orange), set(15)));
		}
	...
	public void action() {
		...
		btn1.addActionListener(e->{
			...
			if (txt1.getText().isBlank() || txt2.getText().isBlank()) {
				...
				Update("insert into review values(null, ?,?,?,?,?,?);", txt1.getText(), txt2.getText(), index + "", member.get(0).get(0), data.get(5), data.get(0));
