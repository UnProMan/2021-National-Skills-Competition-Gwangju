...
public class MenuInsert extends JDialog implements Base{
	...
	public MenuInsert(String mno) {
		
		this.mno = mno;
		
		SetDial(this, "메뉴추가", DISPOSE_ON_CLOSE, 500, 600);
		design();
		action();
		setVisible(true);
	...
	public void design() {
		...
		p2.add(img = get(new JLabel(""), set(new LineBorder(Color.black)), set(160, 140), setb(Color.white)), "West");
		p2.add(p3);
		img.setOpaque(true);
	...
	public void com() {
		...
		Query("select * from type where seller = ?;", temp, member.get(0).get(0));
	...
	public void action() {
		...
		btn3.addActionListener(e->{
			...
			if (txt1.getText().isBlank() || txt2.getText().isBlank() || txt3.getText().isBlank() || txt4.getText().isBlank()) {
				...
				Query("select * from menu where name = ? and seller = ?;", list, txt1.getText(), member.get(0).get(0));
				...
				if (!list.isEmpty()) {
					...
					Update("insert into menu values(null, ?,?,?,?,?,?);", txt1.getText(), txt2.getText(), txt3.getText(), txt4.getText(), member.get(0).get(0), temp.get(com1.getSelectedIndex()).get(0));
		...
		img.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				FileDialog f = new FileDialog(MenuInsert.this, "", FileDialog.LOAD);
				f.setFile("*.png");
